...
int main( int argc, char *argv[] ) {
    ...
    const double bnd0 = BoundaryModel(0);
    const double cnt0 = CenterModel(0);
    ...
    if ( (u = (double **) malloc( ny * sizeof(double *) )) == NULL ) {
    ...
    for ( i = 0 ; i < ny ; i++ ) {
	if ( (u[i] = (double *) malloc( nx * sizeof(double) )) == NULL ) {
	    fprintf("memory allocation for u[%d]", i);
	    for ( j = i-1 ; j>=0 ; j-- ) 
		free( u[j] );
	    free( u );
	    return -2;
    ...
    if ( (u1= (double **) malloc( ny * sizeof(double*) )) == NULL ) {
